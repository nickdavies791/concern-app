...
namespace App\Services\SIMS;

use App\Services\Interfaces\MISInterface;
use App\Token;
use GuzzleHttp\Client;

class Assembly implements MISInterface
{
    /**
     * The Assembly Client ID
     *
     * @var $clientId
     */
    protected $clientId;
    ...
    /**
     * Get OAuth details from Assembly API.
     *
    ...
    /**
     * Refresh access token in database.
     *
     * @param Token $token
    ...
    protected function refreshToken(Token $token)
    {
        ...
        $refreshedToken = $token->update(['secret' => $assembly->access_token]);

        return $refreshedToken ? true : false;
    }

    /**
     * Configure client to access Assembly API.
     *
     * @return Client
     */
